          ...
          }
          {
            status == "success" && (
                <div className="w-full flex flex-col lg:flex-row p-10">
                     <h1 className="text-2xl font-bold text-center md:text-black lg:hidden">{product.name}</h1>
                    ...
                    <div className="w-full lg:w-[50%] h-full flex flex-col justify-center items-center mt-10 gap-4">
                        ...
                        <p className="mt-[30px] text-justify md:text-black">{product.description}</p>
                        <p>Category : {product.category}</p>
                        {
                           product.labelledPrice > product.price ? 
                           <div className="flex gap-3 items-center">
                            <p className="text-lg text-secondary font-semibold line-through">LKR {product.labelledPrice.toFixed(2)}</p>
                            <p className="text-lg text-white md:text-black font-semibold">LKR {product.price.toFixed(2)}</p>
                           </div> : 
                           <p className="text-lg text-white md:text-black font-semibold">LKR {product.price.toFixed(2)}</p>
                        }
                        ...
                        <div className="w-full h-[40px] flex gap-4 mt-[60PX]">
                           ...
                           <Link to="/checkout" state={[{
                              ...
                              quantity : 1
                           }]}className="w-[50%] h-full border border-accent text-white font-semibold hover:bg-accent hover:text-white flex justify-center md:text-black items-center">Buy Now</Link>
                        </div>
                    </div>
                </div>
            )
